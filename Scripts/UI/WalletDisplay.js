...
WalletDisplay.attributes.add('connectButtonEntity', {
    ...
    description: 'The Button Element entity used for connecting and disconnecting.'
});
...
WalletDisplay.prototype.initialize = function() {
    ...
    } else if (this.app.services?.get) { // Fallback to registry
        this.authService = this.app.services.get('authService');
        if(this.authService) console.warn("WalletDisplay: Using app.services fallback to get AuthService.");
    }

    if (!this.authService) {
        console.error("WalletDisplay: AuthService instance not found. UI will not function correctly.");
        // Disable button if service is missing
        if(this.connectButtonEntity?.button) this.connectButtonEntity.button.enabled = false;
    ...
    this.connectButtonTextElement = this.connectButtonEntity?.findByName('Text')?.element;

    // Validate UI elements
    if (!this.addressTextElement) console.warn("WalletDisplay: Wallet Address Text Entity or Element component not found.");
    if (!this.balanceTextElement) console.warn("WalletDisplay: Wallet Balance Text Entity or Element component not found.");
    if (!this.connectButton) console.warn("WalletDisplay: Connect Button Entity or Button component not found.");
    if (!this.connectButtonTextElement) console.warn("WalletDisplay: Text element child of Connect Button not found (needed for label changes).");
    ...
    if (this.app.config && window.SolanaSDK?.web3) {
        ...
        if (rpcEndpoint) {
            try {
                ...
                this.solanaConnection = new window.SolanaSDK.web3.Connection(rpcEndpoint, 'confirmed');
                console.log("WalletDisplay: Solana connection setup for balance checks using:", rpcEndpoint);
            } catch (e) {
        ...
        console.warn("WalletDisplay: ConfigLoader or Solana SDK not ready during initial connection setup.");
...
WalletDisplay.prototype.onConnectClick = function() {
    if (!this.authService) {
        console.error("WalletDisplay: AuthService not available for connect click.");
        return;
    }
...
WalletDisplay.prototype.updateDisplay = function() {
    if (!this.authService) {
        ...
        if(this.addressTextElement) this.addressTextElement.text = "Auth Service Error";
        if(this.balanceTextElement) this.balanceTextElement.text = "";
        if(this.connectButtonTextElement) this.connectButtonTextElement.text = "Error";
...
WalletDisplay.prototype.fetchAndUpdateBalance = async function(address) {
    ...
    if (!this.solanaConnection) {
        console.warn("WalletDisplay: Cannot fetch balance, Solana connection not available.");
        if(this.balanceTextElement) this.balanceTextElement.text = "Balance: N/A (RPC)";
        return;
    }
     if (!address) {
    ...
    if(this.balanceTextElement) this.balanceTextElement.text = "Balance: Fetching...";
    ...
    try {
        ...
        if (this.balanceTextElement) {
            ...
            if (this.authService && this.authService.isAuthenticated() && this.authService.getWalletAddress() === address) {
                 this.balanceTextElement.text = `Balance: ${balanceSOL.toFixed(4)} SOL`;
            } else {
                 console.log("WalletDisplay: Auth state or address changed during balance fetch, discarding result.");
...
WalletDisplay.prototype.formatAddress = function(address) {
    if (!address || typeof address !== 'string' || address.length < 8) return "Invalid Address";
...
WalletDisplay.prototype.formatError = function(errorMsg) {
    ...
    if (errorMsg.includes("Verification failed")) return "Auth Verification Failed";
