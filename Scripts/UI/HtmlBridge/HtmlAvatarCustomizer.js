...
HtmlAvatarCustomizer.prototype._ensureTextureAsset = function (asset, callback) {
    if (!asset) return;
    if (asset.resource) {
        callback(asset);
    } else {
    ...
    if (applyBtn) {
        applyBtn.addEventListener('click', function () {
            self.callbacks.apply.forEach(function (fn) {
                try { fn(); } catch (err) { console.error('Avatar apply callback error', err); }
            });
            self.close();
        });
    }
    ...
    if (cancelBtn) {
        cancelBtn.addEventListener('click', function () {
            self.callbacks.cancel.forEach(function (fn) {
                try { fn(); } catch (err) { console.error('Avatar cancel callback error', err); }
            });
            self.close();
        });
    }
...
HtmlAvatarCustomizer.prototype._createBridge = function () {
    var self = this;
    return {
        onNext: function (slot, handler) { self._registerSlotCallback('next', slot, handler); },
        onPrev: function (slot, handler) { self._registerSlotCallback('prev', slot, handler); },
        onApply: function (handler) { if (handler) self.callbacks.apply.push(handler); },
        onCancel: function (handler) { if (handler) self.callbacks.cancel.push(handler); },
        setSelection: function (slot, info) { self._setSelection(slot, info); },
        setBusy: function (slot, state) { self._setBusy(slot, state); },
        setRecipe: function (recipe) { self._setRecipeSummary(recipe); },
        showRateLimit: function (remainingMs) { self._showRateLimit(remainingMs); },
        open: function () { self.open(); },
        close: function () { self.close(); },
        toggle: function () { if (self.isOpen) self.close(); else self.open(); },
        isOpen: function () { return self.isOpen; }
    };
};

HtmlAvatarCustomizer.prototype._registerSlotCallback = function (type, slot, handler) {
    if (!handler) return;
...
HtmlAvatarCustomizer.prototype._emitSlot = function (type, slot) {
    ...
    list.forEach(function (fn) {
        try { fn(); } catch (err) { console.error('HtmlAvatarCustomizer callback error', err); }
    });
};
...
HtmlAvatarCustomizer.prototype._setSelection = function (slot, info) {
    ...
    if (refs.progressFill) {
        var percent = total ? (displayIndex / total) * 100 : 0;
...
HtmlAvatarCustomizer.prototype._setBusy = function (slot, state) {
    ...
    if (!refs) return;
    if (refs.container) {
        refs.container.classList.toggle('is-busy', !!state);
    }
    if (refs.busyEl) {
        refs.busyEl.classList.toggle('hidden', !state);
    }
};
...
HtmlAvatarCustomizer.prototype._showRateLimit = function (remainingMs) {
    ...
    this.rateLimitEl.textContent = seconds > 0 ? ('Please wait ' + seconds + 's before applying again.') : '';
    this.rateLimitEl.classList.remove('hidden');
    var self = this;
    this._rateLimitTimer = setTimeout(function () {
        if (self.rateLimitEl) {
            self.rateLimitEl.classList.add('hidden');
        }
